...
public class OAuthUtils {
	...
	public static String getAuthorizationCode(OAuth2Details oauthDetails) {
		...
		String location = oauthDetails.getAuthenticationServerUrl() + "?";

		String state = oauthDetails.getState();

		location += OAuthConstants.RESPONSE_TYPE + "=" +	OAuthConstants.CODE;
		location += "&" + OAuthConstants.CLIENT_ID + "=" +	oauthDetails.getClientId();
		location += "&" + OAuthConstants.REDIRECT_URI + "=" +	oauthDetails.getRedirectURI();


		if (isValid(oauthDetails.getScope())) {
			location += "&" + OAuthConstants.SCOPE + "=" +	oauthDetails.getScope();
		}

		
		if (isValid(state)) {
			location += "&" + OAuthConstants.STATE + "=" +	oauthDetails.getState();
				}
